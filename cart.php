...
if (isset($_POST['coupon_submit'])) {
    ...
    // Query to fetch coupon details
    $coupon_query = "SELECT * FROM coupons WHERE coupon_code = '$coupon_code'";
    $coupon_res = mysqli_query($conn, $coupon_query);
    $coupons = mysqli_fetch_array($coupon_res);
    $coupon_count = mysqli_num_rows($coupon_res);

    if ($coupon_count > 0 && isset($coupons['coupon_percentage'])) {
        $coupon_percentage = (float)$coupons['coupon_percentage'];
        ...
        if ($coupon_percentage > 0) {
            ...
            $discount_amount = ($total * $coupon_percentage) / 100;

            // Update session variables
            $_SESSION['coupon_code'] = $coupon_code;
            $_SESSION['coupon_amount'] = $discount_amount;
            $_SESSION['coupon_status'] = 'Y';

            echo "<script>alertify.success('Coupon Applied Successfully: $coupon_percentage% OFF');</script>";
            echo "<script>window.location.href='cart.php';</script>";
        } else {
            echo "<script>alertify.error('Invalid Coupon Percentage');</script>";
        }
    } else {
        // Invalid coupon
        $_SESSION['coupon_code'] = '';
        $_SESSION['coupon_amount'] = 0;
        $_SESSION['coupon_status'] = 'N';
        echo "<script>alertify.error('Invalid Coupon Code');</script>";
    }
    unset($_POST['coupon_submit']);
}
...
if (isset($_SESSION['coupon_status']) && $_SESSION['coupon_status'] == "Y") {
    $coupon_amount = $_SESSION['coupon_amount'];
} else {
    $coupon_amount = 0;
}
        ...
        <!-- continue shopping start  -->
            ...
            <div class="col-md-4">
                ...
                <div class="card-body">
                    ...
                    <div class="prodect-discription-cartt">
                           <!-- <a href="checkout.php"> -->
                           <button class="CartBtnn btn btn-danger" type='submit' name='checkout_submit'>
